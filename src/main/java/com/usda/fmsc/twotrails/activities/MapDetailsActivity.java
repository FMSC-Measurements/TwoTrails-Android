...
//TODO update path
//TODO update details, show progress bar while updating, ask for overwrite
...
public class MapDetailsActivity extends CustomToolbarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        arcGisMapLayer = getIntent().getParcelableExtra(Consts.Activities.Data.MAP_DATA);
                    ...
                    case R.id.amdMenuDelete: {
                        new AlertDialog.Builder(MapDetailsActivity.this)
                                ...
                                .setPositiveButton(R.string.str_delete, new DialogInterface.OnClickListener() {
                                    @Override
                                    public void onClick(DialogInterface dialog, int which) {
                                        ArcGISTools.deleteMapLayer(arcGisMapLayer.getId(), true, new ISimpleEvent() {
                                            @Override
                                            public void onEventTriggerd(Object o) {
                                                setUpdated(false);

                                                Intent i = new Intent();
                                                i.putExtra(Consts.Activities.Data.MAP_DATA, arcGisMapLayer);

                                                MapDetailsActivity.this.setResult(Consts.Activities.Results.MAP_DELETED, i);
    ...
    private void setValues(ArcGisMapLayer agml) {
        if (ivStatusIcon != null)
            ivStatusIcon.setImageDrawable(AndroidUtils.UI.getDrawable(this,
                    arcGisMapLayer.isOnline() ? R.drawable.ic_online_primary_36 :
        ...
        if (!agml.hasValidFile())
            tvFile.setTextColor(AndroidUtils.UI.getColor(this, R.color.red_800));

        txtLoc.setText(agml.getLocation());

        txtDesc.setText(agml.getDescription());

        tvScaleMin.setText(agml.getMinScale() < 0 ? StringEx.Empty : Double.toString(agml.getMinScale()));

        tvScaleMax.setText(agml.getMaxScale() < 0 ? StringEx.Empty : Double.toString(agml.getMaxScale()));
    }
    ...
    public void finish() {
        if (updated) {
            Intent i = new Intent();
            i.putExtra(Consts.Activities.Data.MAP_DATA, arcGisMapLayer);
            setResult(Consts.Activities.Results.MAP_UPDATED, i);
        }
